...
main() {
  for __sql_file in ./sql/2_query/*.sql; do
    ...
    ./psql.sh "${__temp_file_path}" | grep -v 'You are now connected to database' > "${__file_text}" || return "$?"
    ...
    # ./psql.sh "${__temp_file_path}" | sed -En 's/^   (.*[^ ]+) +\+$/\1/p' > "${__file_json}" || return "$?"
...
${__response}" >&2
    ...
    echo "Getting HTML..." >&2
    __html="$(./venv/bin/python ./explain_query_plan.py "https://explain.tensor.ru${__url_explained}#parsed")" || return "$?"
